...
def test_user_profile_tool():
    ...
    try:
        ...
        if profile.get("found"):
            ...
            # Stage first — the same week number means "weeks pregnant" for an NP
            # user and "weeks postpartum" for a PP one.
            print(f"   Stage: {profile.get('stage')} (category {profile.get('user_category')})")
            print(f"   Week: {profile.get('week', 0)}, day {profile.get('days', 0)}")
