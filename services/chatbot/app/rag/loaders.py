...
# Constants (6, 17)
DEFAULT_MAX_FILE_SIZE_MB = 30  # Maximum file size to load (prevents memory issues)
...
def discover_files(
    folder_path: Path, file_types: List[str], recursive: bool = False, skip_hidden: bool = True
) -> List[Path]:
    """
...
def load_single_file(file_path: Path, max_size_bytes: int) -> Optional[Document]:
    """
            ...
            for enc in encodings_to_try:
                try:
                    ...
                    if not file_docs or not isinstance(file_docs, list):
                        logger.warning(f"Loader returned invalid result for {file_path.name}")
                        continue

                    # Successfully loaded
                    if len(file_docs) > 0:
                        # PyPDFLoader returns one Document per page; merge all pages
                        # so the downstream splitter can create proper overlapping
                        # chunks that span page boundaries.
                        if file_path.suffix.lower() == ".pdf" and len(file_docs) > 1:
                            combined_content = "\n\n".join(
                                d.page_content
                                for d in file_docs
                    ...
                    logger.debug(f"Error loading {file_path.name} with encoding {enc}: {str(e)}")
                    continue
...
def enhance_metadata(doc: Document, file_path: Path, file_size: int, encoding: str) -> Document:
    """
...
def remove_duplicates(docs: List[Document]) -> List[Document]:
    ...
    if duplicates_removed > 0:
        logger.warning(f"Removed {duplicates_removed} duplicate document(s) from {len(docs)} total")
...
def load_files_parallel(
    file_paths: List[Path], max_size_bytes: int, max_workers: int = MAX_WORKERS
) -> List[Document]:
    """
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_path = {
            executor.submit(load_single_file, path, max_size_bytes): path for path in file_paths
        }
...
def load_documents(
    ...
    file_paths = discover_files(
        folder_path, file_types, recursive=recursive, skip_hidden=skip_hidden
    )
