...
class _RetrievalMetrics:
    ...
    def record_query(
        self, had_results: bool, above_threshold: bool, best_score: float, query_time_ms: float
    ):
...
class RAGRetriever:
    ...
    def __init__(
        ...
        embedding_model_load_path: Optional[str] = None,
    ):
        """
        ...
        Args:
            ...
            embedding_model_load_path: Local directory to LOAD the model weights
                from (e.g. one downloaded from GCS at boot). The model *identity*
                used for the index-compatibility check stays `embedding_model`
                (the name), so the downloaded folder still matches an index built
                as "BAAI/bge-m3". When None, the model is loaded by name.
        """
        # Configuration
        self.embedding_model_name = embedding_model or settings.embeddings_model
        # Where to actually read the weights from — a local path takes priority
        # over the name so a GCS-provisioned model never triggers an HF download.
        self._embedding_load_path = embedding_model_load_path
    ...
    def _get_embeddings(self) -> HuggingFaceEmbeddings:
        ...
        if self._embeddings is None:
            # Load weights from the local path if one was provisioned (GCS boot
            # download); otherwise load by name (baked cache / HF). Identity for
            # the index check is unchanged (self.embedding_model_name).
            model_ref = self._embedding_load_path or self.embedding_model_name
            logger.info(
                f"Loading embeddings model: {model_ref}"
                + (f" (identity: {self.embedding_model_name})" if self._embedding_load_path else "")
            )
            start_time = time.time()

            try:
                # normalize_embeddings=True guarantees unit-length vectors for ANY
                # model (English or multilingual). _normalize_score()'s exact
                # L2->cosine identity (sim = 1 - d^2/2) depends on this; without it
                # a model like bge-m3 would silently produce wrong similarity scores.
                # settings.ingest_progress (from .env, a shell env var, or the CI
                # -e INGEST_PROGRESS=1) shows a tqdm progress bar with ETA while
                # embedding the corpus, so the otherwise-silent ~hour-long build
                # shows live progress. Off at runtime (each query is one tiny batch).
                # NOTE: use the dedicated `show_progress` arg, NOT encode_kwargs —
                # embed_documents passes show_progress_bar=self.show_progress to
                # encode(), so encode_kwargs["show_progress_bar"] would be a
                # duplicate keyword.
                self._embeddings = HuggingFaceEmbeddings(
                    model_name=model_ref,
                    encode_kwargs={"normalize_embeddings": True},
                    show_progress=settings.ingest_progress,
                )
    ...
    def build_index(
        self, docs: List[Document], save: bool = True, incremental: bool = False
    ) -> IndexMetadata:
        """
        ...
        if not incremental:
            ...
            try:
                ...
                self._index = FAISS.from_documents(documents=chunks, embedding=embeddings)
    ...
    def load_index(self) -> bool:
        ...
        try:
            # Issue #9: Load metadata first (cheap JSON read).
            if metadata_file.exists():
                ...
                # Refuse to use an index built with a different embedding model.
                # The vector dimensions won't match (e.g. MiniLM=384 vs bge-m3=1024),
                # so every query would throw. Fail loud and clear instead, telling
                # the operator to rebuild. Checked BEFORE loading the embedding
                # model so a stale index doesn't trigger a needless (large) download.
                if self._metadata.embedding_model != self.embedding_model_name:
                    logger.error(
                        "Embedding model mismatch: index was built with "
                        f"'{self._metadata.embedding_model}' but the app is configured "
                        f"for '{self.embedding_model_name}'. The index is incompatible. "
                        "Rebuild it with `python ingest_data.py` before RAG can work."
                    )
                    return False
            else:
            ...
            self._index = FAISS.load_local(
                str(index_file), embeddings, allow_dangerous_deserialization=True
            )
    ...
    def _normalize_score(self, distance: float) -> float:
        ...
        Note on L2 calibration:
        HuggingFaceEmbeddings/sentence-transformers output unit-normalized
        vectors (verified: ||v|| ≈ 1.0), so for L2 distance d between two
        unit vectors there is an exact identity with cosine similarity:
            d^2 = ||a-b||^2 = ||a||^2 + ||b||^2 - 2*a.b = 2 - 2*cos_sim
            => cos_sim = 1 - d^2/2
        We use this exact relationship rather than an empirical approximation.
        """
        if self.distance_metric == "cosine":
            ...
            # Exact L2-to-cosine-similarity conversion for unit-normalized vectors.
    ...
    def query(
        self, question: str, k: Optional[int] = None, threshold: Optional[float] = None
    ) -> RetrievalResult:
        """
    ...
    def query_best_of(
        ...
        """
        Run several query variants and return the highest-scoring result.

        Used for dual retrieval: e.g. the raw user query (multilingual model)
        plus an English translation. This removes the single-point-of-failure —
        if one variant retrieves poorly (a bad translation, or a romanized
        query the embedder handles weakly), the other can still carry it.

        Empty/duplicate variants are skipped. Returns an empty result only if
        every variant was empty/invalid.
        """
        ...
        for q in questions:
            ...
            if key in seen:
                continue
            seen.add(key)
            res = self.query(q, k=k, threshold=threshold)
...
# ============================================
# SHARED SINGLETON (load model + index ONCE per process)
# ============================================
#
# The chat pipeline previously did `RAGRetriever(); load_index()` on EVERY
# request, which re-loaded the ~2 GB embedding model into RAM each time. This
# singleton loads the model (downloading it from GCS first, if configured) and
# the FAISS index exactly once per container, and every request reuses it.
#
# Warmed at app startup (app/api/main.py) so the first real request is fast, and
# guarded by a lock so a request that races startup builds it exactly once.

import threading as _threading

_shared_retriever: Optional[RAGRetriever] = None
_shared_retriever_lock = _threading.Lock()


def get_shared_retriever() -> RAGRetriever:
    """Return the process-wide retriever, building it once on first call.

    Downloads the embedding model from GCS (when MODEL_GCS_URI is set), then
    loads the model and FAISS index. Thread-safe and idempotent.
    """
    ...
    with _shared_retriever_lock:
        if _shared_retriever is not None:  # re-check inside the lock
            return _shared_retriever

        # Provision the model (GCS → local dir) before constructing the retriever
        # so weights load from local disk, never over the network at runtime.
        from app.rag.model_provisioner import ensure_model_available

        load_path = ensure_model_available()

        retriever = RAGRetriever(embedding_model_load_path=load_path)
        if not retriever.load_index():
            # load_index already logged the reason. Raise so startup warmup and
            # /readyz surface the failure instead of silently serving no-RAG.
            raise RuntimeError(
                "Failed to load FAISS index for the shared retriever "
                "(see logs — likely a missing index or model/index mismatch)."
            )
        _shared_retriever = retriever
        logger.info("Shared retriever ready (model + index loaded once).")
        return _shared_retriever
...
def build_or_load_index(docs: Optional[List[Document]] = None) -> FAISS:
    ...
    if docs:
        ...
        if not loaded:
            ...
            placeholder_doc = Document(page_content="placeholder", metadata={"placeholder": True})
...
def query_with_fallback(
    question: str, k: Optional[int] = None
