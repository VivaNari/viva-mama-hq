...
CREDIT_CARD_RE = re.compile(r"\b\d{4}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{4,7}\b")
...
MEDICAL_REFUSAL = (
    ...
    "Respond with general perinatal wellness education, self-care tips, "
...
PROMPT_INJECTION_REFUSAL = (
    ...
    "I can only discuss perinatal wellness topics.\n\n"
    "— *Wellness information only; not medical advice.*"
)

# "Perinatal", not "postpartum": VivaMama serves users who are still pregnant, and this
# wording is what tells a user what she may ask about. Kept in step with the RELEVANCE
# GATE in chat_pipeline_mcp.SYSTEM_PROMPT, which is where the live topic decision is made
# (see the note on _metrics above — scope checking moved into the main LLM prompt).
OFF_TOPIC_REFUSAL = (
    "I'm sorry, but I'm a perinatal wellness assistant. "
    "I can only help with questions related to pregnancy, postpartum care, recovery, "
    "and wellness.\n\n"
    "— *Wellness information only; not medical advice.*"
)
...
def check_prompt_injection(text: str) -> Tuple[bool, list]:
    ...
    if matched_patterns:
        ...
        logger.warning(f"Prompt injection detected. Matched patterns: {len(matched_patterns)}")
...
def check_medical_scope(text: str) -> Tuple[bool, list]:
    ...
    if matched_patterns:
        ...
        logger.info(f"Medical scope violation detected. Matched patterns: {len(matched_patterns)}")
...
def enforce_scope(text: str) -> Tuple[str, Dict[str, Any]]:
    ...
    """
    notes: Dict[str, Any] = {"blocked": False, "offtopic": False, "matched": []}
