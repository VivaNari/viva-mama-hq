...
class _LLMMetrics:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "success_rate": (
                self.calls_success / self.calls_total if self.calls_total > 0 else 0.0
            ),
    ...
    def call(self, func, *args, **kwargs):
        ...
        try:
            ...
            if self.state == "HALF_OPEN":
                ...
                logger.error(
                    f"Circuit breaker: CLOSED → OPEN ({self.failure_count} consecutive failures)"
                )
                self.state = "OPEN"
...
class _GroqClientManager:
    ...
    def get_client(
        ...
        # Create new client
        logger.info(
            f"Creating new Groq client: model={model}, temp={temperature}, timeout={timeout}s"
        )
...
def _invoke_with_resilience(
    client: BaseChatModel, prompt: str, model: str, operation_type: str = "chat"
) -> Any:
    """
    ...
    try:
        ...
        response_preview = (
            ...
            if len(str(response.content)) > 200
        ...
        _metrics.record_call(success=False, latency_ms=elapsed_ms, error_type="rate_limit")
        ...
        _metrics.record_call(success=False, latency_ms=elapsed_ms, error_type="timeout")
        ...
        _metrics.record_call(success=False, latency_ms=elapsed_ms, error_type="connection")
        ...
        _metrics.record_call(success=False, latency_ms=elapsed_ms, error_type="api_error")
        ...
        _metrics.record_call(success=False, latency_ms=elapsed_ms, error_type="unknown")
