...
import RecommendationHistoryService from "../src/services/recommendations/recommendation-history.service";
...
describe("RecommendationHistoryService", () => {
    ...
    it("create() saves history", async () => {
        ...
        const payload = {
            ...
            finalScore: 85,
            zone: "GREEN" as const,
            weakestCategory: "emotional" as const,
            breastfeeding: true,
            recommendationId: "R1",
            categoryScores: {
        ...
        const result = await RecommendationHistoryService.create(payload);
    ...
    it("create() throws when DB fails", async () => {
        ...
        await expect(
            RecommendationHistoryService.create({
                userId: "U1",
                week: 3,
                finalScore: 85,
                zone: "GREEN",
    ...
    it("getUserHistory() returns array", async () => {
        ...
        const result = await RecommendationHistoryService.getUserHistory("U1", 2);
    ...
    it("getHistoryByWeek() returns single record", async () => {
        ...
        const result = await RecommendationHistoryService.getHistoryByWeek("U1", 3);
