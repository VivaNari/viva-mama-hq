...
jest.mock(require.resolve("../src/api/v1/controllers/payments/payment.controller"), () => jest.fn().mockImplementation(() => ({
    createOrder: () => { },
    verifyPayment: () => { },
    selectFreePlan: (req, res) => {
        if (!req.body?.userId)
...
describe("POST /api/v1/subscribe/select-free-plan", () => {
    it("400 when userId missing", async () => {
        const res = await (0, supertest_1.default)(app_1.default).post("/api/v1/subscribe/select-free-plan").send({});
...
//# sourceMappingURL=payments_selectFreePlan.test.js.map