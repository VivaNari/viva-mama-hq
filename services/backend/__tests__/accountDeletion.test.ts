...
async function seed() {
    ...
    // And a milestone they reached.
    await milestoneLogModel.create({
        userId: uid,
        childId: new Types.ObjectId(),
    ...
    // And a day of that child's feeds.
...
describe("AccountDeletionService", () => {
    it("removes the user and every collection keyed to them", async () => {
        ...
        expect(await feedingLogModel.countDocuments({ userId: uid })).toBe(0);
