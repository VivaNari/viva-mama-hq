...
export const usePayment = ({
    ...
    const processFreePlan = useCallback(async (): Promise<boolean> => {
        ...
        dispatch({ type: 'SET_PAYMENT_STATUS', payload: PaymentStatus.CREATING_ORDER });
     ...
     */
    const createOrder = useCallback(async (): Promise<IPaymentOrderResponse['data'] | null> => {
    ...
    const verifyPayment = useCallback(async (
        paymentResponse: IRazorpaySuccessResponse
    ): Promise<void> => {
        dispatch({ type: 'SET_PAYMENT_STATUS', payload: PaymentStatus.VERIFYING });
    ...
    const processPayment = useCallback(async (): Promise<boolean> => {
        ...
        try {
            ...
            dispatch({ type: 'SET_PAYMENT_STATUS', payload: PaymentStatus.AWAITING_PAYMENT });
            ...
            if (error && typeof error === 'object' && 'code' in error) {
                ...
                dispatch({ type: 'SET_ERROR', payload: parseRazorpayError(razorpayError) });
            } else if (error instanceof Error && error.message.includes('verification')) {
                dispatch({ type: 'SET_ERROR', payload: createVerificationError(error) });
                ...
                dispatch({ type: 'SET_ERROR', payload: createOrderCreationError(error) });
            }
